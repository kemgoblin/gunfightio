...
# Variables for bob and sway effect
var base_sway_amount := 0.0002  # Minimal movement when idle
var max_sway_amount := 0.0005  # Increased movement when sprinting
var base_bob_amount := 0.0002
var max_bob_amount := 0.0005
var sway_speed := 2.0
var bob_speed := 3.0
var t_sway := 0.0
var t_bob := 0.0
var movement_factor := 0.0  # Smoothed movement factor
...
func _movement_process(delta):
	...
	if health <= 0: input_dir = Vector2.ZERO
	...
	if not input_dir == Vector2.ZERO:    # This means "if player pushing left stick"
		if speed < true_max_speed:
			speed += accel
		else:
			speed -= accel
	else:
	...
	# Define gravity now, to protect it from override.
	var gravity := velocity.y
	
	# Calculate gravity.
	var world_gravity = ProjectSettings.get_setting("physics/3d/default_gravity") * delta
	if not is_on_floor(): gravity -= world_gravity
	else: gravity = GRAVITY_COLLIDE
...
func set_ads(is_aiming: bool):
	...
	if ads_mode:
		...
		hud.fade_reticle(false)  # Fade out the reticle when aiming.
	else:
		ads_fov_target = ads_fov_max
		hud.fade_reticle(true)   # Fade in the reticle when not aiming.
...
func _handle_bob_and_sway(delta: float) -> void:
	# Increase time variables for smooth oscillation
	t_sway += delta * sway_speed
	t_bob += delta * bob_speed

	# Target movement factor based on speed
	var target_movement_factor = clamp(speed / max_walk_speed, 0.0, 1.0)

	# Smoothly interpolate between previous and new movement factor
	movement_factor = lerp(movement_factor, target_movement_factor, delta * 3.0)  # Adjust multiplier for smoothness

	# Calculate sway (side to side movement)
	var sway_offset = sin(t_sway) * lerp(base_sway_amount, max_sway_amount, movement_factor)

	# Calculate bob (up and down movement)
	var bob_offset = cos(t_bob) * lerp(base_bob_amount, max_bob_amount, movement_factor)

	# Apply bob and sway effect to the arms rig
	arms_rig.transform.origin.x = sway_offset
	arms_rig.transform.origin.y = bob_offset
